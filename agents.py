...
def researcher_node(state: ArticleWorkflowState) -> dict:
    ...
    # Poprawiono błąd składniowy (nazwa zmiennej nie może zawierać spacji)
    ...
    print(f"--- 🕵️ Scrapowanie treści z {len(urls)} stron... ---")
    scraped_content = []
    for url in urls:
        try:
...
def assembler_node(state: ArticleWorkflowState) -> dict:
    ...
    full_article = f"# {h1_title}\n\n" + "\n\n".join(f"## {s['title']}\n\n{s['draft']}" for s in state["outline"] if s["is_approved"] and s["draft"])
    print("✅ Wszystkie sekcje zostały złożone w jeden artykuł.")
    return {"final_article": full_article}

def final_editor_node(state: ArticleWorkflowState) -> dict:
    print("--- ✏️ Agent: Final Editor ---")
    llm = state["llm"]
    prompt = f"""Jesteś redaktorem końcowym. Twoim zadaniem jest ostatni szlif poniższego, kompletnego artykułu. Popraw błędy gramatyczne i stylistyczne, powtórzenia wyrazów lub często powtarzających się zwrotów. Upewnij się, że przejścia między sekcjami są płynne i że cały tekst jest spójny. Sprawdź, czy artykuł jest UX-friendly oraz user-friendly (zawiera wypunktowania i pogrubienia - ale nie w nadmiarze). Nie zmieniaj sensu ani tonu. Zwróć tylko i wyłącznie finalną, 'wypolerowaną' wersję artykułu.
...
def should_continue_writing(state: ArticleWorkflowState) -> str:
    ...
    # Najpierw sprawdź warunek wyjścia z pętli
    ...
    # Jeśli nie wszystkie są zatwierdzone, znajdź następną sekcję do pracy
    work_in_progress_section = next((s for s in state["outline"] if not s.get("is_approved")), None)

    # To nie powinno się zdarzyć, ale to dobre zabezpieczenie
    if not work_in_progress_section:
        print("--- ‼️ Błąd logiczny: Brak niezatwierdzonych sekcji, ale pętla pisania kontynuuje. Wymuszam składanie. ---")
        return "assemble_article"

    # Sprawdź, czy obecna sekcja osiągnęła limit poprawek
    if work_in_progress_section.get("revision_count", 0) >= 3:
        ...
        # Po wymuszonej akceptacji, natychmiast sprawdź ponownie, czy to już koniec
        if all(s.get("is_approved", False) for s in state["outline"]):
            print("--- ✅ Wszystkie sekcje zatwierdzone po wymuszonej akceptacji. Przechodzę do składania. ---")
            return "assemble_article"

    # Jeśli jesteśmy tutaj, oznacza to, że musimy kontynuować pętlę pisania/krytyki
